...
@Component({
  ...
  styleUrls: ['./tab-first.component.css']
})
export class TabFirstComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    this.tabService.setResponseCode.pipe(takeWhile(() => this.alive)).subscribe(response => {
      try
      {
        ...
        console.log('Tab characters' + this.characters);
      }
      catch(e)
      {
        console.log(e);
  ...
  refreshList(){
    try
    {
      this.characters = this.tabService.charactersTop;
    }
    catch(err)
    {
      console.log(err);
